...
#ifdef __STDC__
static rx_Bitset
inverse_translation (int cset_size, char * valid, rx_Bitset cache, unsigned char * translate, int c)
#else
static rx_Bitset
inverse_translation (cset_size, valid, cache, translate, c)
     int cset_size;
     char * valid;
     rx_Bitset cache;
...
compile_range (int cset_size, rx_Bitset cs, const char ** p_ptr, const char * pend, unsigned char * translate, unsigned long syntax, rx_Bitset inv_tr, char * valid_inv_tr)
#else
static reg_errcode_t
compile_range (cset_size, cs, p_ptr, pend, translate, syntax, inv_tr, valid_inv_tr)
     int cset_size;
     rx_Bitset cs;
     const char ** p_ptr;
     const char * pend;
...
#endif
{
  ...
  for (this_char = range_start; this_char <= range_end; this_char++)
    {
      rx_Bitset it =
	inverse_translation (cset_size, valid_inv_tr, inv_tr, translate, this_char);
	    ...
	    append = node;
                ...
                if (p == pend)
		  {
			...
			goto error_return;
                    ...
                    PATFETCH (c1);
		    {
		      rx_Bitset it = inverse_translation (cset_size,
							  validate_inv_tr,
							  inverse_translate,
							  translate,
							  c1);
		    ...
		    goto error_return;
                ...
                if (c == '-' 
                  ...
                  {
                    reg_errcode_t ret
                      = compile_range (cset_size, cs, &p, pend, translate, syntax,
		    ...
		    /* Move past the `-'.  */
                    ...
                    ret = compile_range (cset_size, cs, &p, pend, translate, syntax,
					 inverse_translate, validate_inv_tr);
                    if (ret != REG_NOERROR)
		      {
			compile_error = ret;
	...
	case '|':
	  ...
	  if ((syntax & RE_INTERVALS) && (syntax & RE_NO_BK_BRACES))
	      ...
	      if (*last_expression)
		{
		  ...
		  concat = rx_mk_r_binop (r_concat, *last_expression, 0);
		  if (!concat)
		    goto space_error;
		...
		*top_expression = alt;
		...
		PATFETCH (c);
		{
		  ...
		  for (x = 0; x < 256; ++x)
			...
			{
			  rx_Bitset it =
			    inverse_translation (cset_size, validate_inv_tr,
						 inverse_translate,
						 translate, x);
