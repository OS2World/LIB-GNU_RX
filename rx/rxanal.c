...
#else
int
rx_posix_analyze_rexp (subexps, n_subexps, node, id)
...
#endif
{
  if (node)
      ...
      if (node->type == r_parens)
	{
	  ...
	  ++*n_subexps;
	  if (!*subexps)
	    *subexps = (struct rexp_node **)malloc (sizeof (struct rexp_node *) * *n_subexps);
	  else
	    *subexps = (struct rexp_node **)realloc (*subexps,
						     sizeof (struct rexp_node *) * *n_subexps);
	}
...
#endif
{
  ...
  if (   frame->rx->start_set
      && (frame->rx->start_set->starts_for == frame->rx)
      && (frame->rx->start_set->id == frame->rx->rx_id))
  ...
  if (   start_contents->superstate
  ...
  else
    {
      ...
      frame->state = rx_superstate (frame->rx, start_contents);
...
#ifdef __STDC__
enum rx_answers
rx_match_here_p (struct rx_classical_system * frame,
		 unsigned char * burst,
		 int len)
#else
enum rx_answers
rx_match_here_p (frame, burst, len)
     struct rx_classical_system * frame;
     unsigned char * burst;
     int len;
#endif
...
#ifdef __STDC__
enum rx_answers
rx_fit_p (struct rx_classical_system * frame,
		 unsigned char * burst,
		 int len)
#else
enum rx_answers
...
#else
enum rx_answers
rx_longest (last_matched, base_addr, frame, burst, len)
     int * last_matched;
     int base_addr;
     struct rx_classical_system * frame;
     unsigned char * burst;
...
#endif
{
  ...
  while (len--)
      ...
      while (!next_table)
	{
      ...
      if (inx->data_2)		/* indicates a final superstate */
	*last_matched = base_addr + len_in - len;
      inx_table = next_table;
      ++burst;
    }
...
#ifdef __STDC__
enum rx_answers
rx_advance (struct rx_classical_system * frame,
